...
def extract_files():
    ...
    """
    data_file = []
    folders = [DATA_DIR+'/'+'train', DATA_DIR+'/'+'test',]
    print(folders)
    for folder in folders:
        class_folders = glob.glob(os.path.join(folder, '*'))

        for vid_class in class_folders:
            class_files = glob.glob(os.path.join(DATA_DIR, folder, vid_class, '*.mp4'))
...
def check_already_extracted(video_parts):
    ...
    train_or_test, classname, filename_no_ext, _ = video_parts
    return bool(os.path.exists(os.path.join(DATA_DIR, train_or_test, classname,
